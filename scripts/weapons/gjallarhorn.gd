...
@export var weapon_data: WeaponData = preload("res://resources/weapons/gjallarhorn_data.tres")
...
@onready var visual_ring: Sprite2D = $VisualRing
...
func _ready() -> void:
	...
	if visual_ring:
		visual_ring.texture = load("res://assets/sprites/weapons/gjallarhorneffect.png")
		visual_ring.show()
...
func _process(delta: float) -> void:
	if visual_ring and visual_ring.texture:
		var target_size = _radius * 2.0
		var base_scale = target_size / float(visual_ring.texture.get_width())
		visual_ring.scale = visual_ring.scale.lerp(Vector2(base_scale, base_scale), delta * 6.0)
		visual_ring.modulate.a = lerpf(visual_ring.modulate.a, 0.8, delta * 3.0)

func get_weapon_id() -> StringName:
	return weapon_data.id if weapon_data else &"gjallarhorn"
...
func _apply_level_stats() -> void:
	...
	if level_data.special_effect == &"knockback":
		_knockback_force = level_data.special_value
	elif level_data.special_effect == &"knockback_stun":
		_knockback_force = level_data.special_value
		_stun_duration = 1.0 # arbitrary short stun
...
func _on_tick_timeout() -> void:
	...
	if visual_ring and visual_ring.texture:
		var target_size = _radius * 2.0
		var base_scale = target_size / float(visual_ring.texture.get_width())
		visual_ring.scale = Vector2(base_scale * 1.2, base_scale * 1.2)
		visual_ring.modulate.a = 1.0
...
func _apply_damage(enemy: CharacterBody2D) -> void:
	...
	if health and health.is_alive():
		...
		# Stun mechanics se houver
		if _stun_duration > 0.0 and enemy.has_method("apply_temporary_slow"):
			enemy.apply_temporary_slow(0.1, _stun_duration)
